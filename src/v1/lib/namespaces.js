...
module.exports.listNamespaces = async (token) => {
  logger.info('inside listNs');
  const usertoken = token;
  ...
  request.get(options, (err, response, body) => {
    ...
    if (err) {
      ...
      request.get(options, (nmerr, nmresponse, nmbody) => {
        ...
        //     console.log(util.inspect(nmobj, {showHidden: false, depth: null}));
        //   }
          logger.info('GET namespaces api successful');
          return nmbody.items;
        }
        logger.error('Unknown get namespaces error');
        return null;
      });
    } else if (response.statusCode === 200 && body && body.items) {
    //   for (let i = 0, l = body.items.length; i < l; i += 1) {
    ...
    //     console.log(util.inspect(obj, {showHidden: false, depth: null}))
    //   }
      logger.info('GET projects api successful');
      // logger.info(body.items);
      return body.items;
    }
    logger.error('Unknown get namespaces error');
    return null;
